...
describe("Transactions", () => {
    ...
    describe("database.executeTransaction", () => {
        it("should execute a transaction and return the result", async () => {
            const result = await db.executeTransaction([], "function (params) {return params;}", { params: "test" });
    ...
    describe35("stream transactions", () => {
        ...
        it("can commit an empty transaction", async () => {
            const trx = await db.beginTransaction(collection);
            {
                ...
                chai_1.expect(id).to.equal(trx.id);
                chai_1.expect(status).to.equal("running");
            }
            {
        ...
        it("can insert two documents sequentially", async () => {
            ...
            chai_1.expect(meta1).to.have.property("_key", "test1");
            chai_1.expect(meta2).to.have.property("_key", "test2");
            const { id, status } = await trx.commit();
            chai_1.expect(id).to.equal(trx.id);
            chai_1.expect(status).to.equal("committed");
            const doc1 = await collection.document("test1");
            chai_1.expect(doc1).to.have.property("_key", "test1");
            const doc2 = await collection.document("test2");
            chai_1.expect(doc2).to.have.property("_key", "test2");
        });
        itRdb("does not leak when inserting a document", async () => {
            ...
            if (doc)
                chai_1.expect.fail("Document should not exist yet.");
            const { id, status } = await trx.commit();
            chai_1.expect(id).to.equal(trx.id);
            chai_1.expect(status).to.equal("committed");
        });
        itRdb("does not leak when inserting two documents sequentially", async () => {
            const trx = await db.beginTransaction(collection);
...
//# sourceMappingURL=04-transactions.js.map